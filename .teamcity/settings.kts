...
object config : BuildType({
    ...
    artifactRules = "**/*.bat"
    buildNumberPattern = "1.1.%build.counter%"

    params {
        param("system.path.macro.MAVEN.REPOSITORY", "env.m2")
        param("env.JDK_17", "y")
        param("env.JDK_16", "t")
    }
    ...
    steps {
        ideaRunner {
            ...
            jvmArgs = "-Xmx256m"
            runConfigurations = "all"
        }
        script {
            enabled = false
            scriptContent = "echo aaaa"
        }
        ideaInspections {
    ...
    dependencies {
        artifacts(AbsoluteId("Manybranches1_Build4")) {
            buildRule = lastPinned("refs/heads/branch1002")
